...
public class GameWorld : MonoBehaviour {
    ...
    GameLevel spawnNextLevel(bool isFirst)
    {
        ...
        if (isFirst)
        {
            next.Init(Vector2.zero, Config.LevelList()[0]);
        }
        else
        {
    ...
    public void OutOfMap()
    {
        ...
        if (m_currentLevel.OverMap(m_camera.GetTargetPos()))
        {
            m_currentLevel = m_nextLevel;
            Vector2 outPos = Vector2.zero;
            foreach (Character c in m_teamA.Members())
            {
                if (m_currentLevel.OutOfMap(c.Pos, out outPos))
                    c.Pos = outPos;
                m_currentLevel.Partition().AddEntity(c);
            }

        }
        Vector2 cameraborder = m_camera.GetCameraPos() - new Vector2(300, 0);
        if (m_previousLevel.OverMap(cameraborder))
        {
